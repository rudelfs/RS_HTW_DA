...
export default function WeatherResult({ t, siteName, onTrackClick, onComplete }) {
  ...
  return (
    <BrowserWindow url="wetter-schnell.de/berlin">
      <div className="font-sans relative bg-slate-50 min-h-[800px] overflow-hidden text-left">
        ...
        <main className={`p-4 md:p-8 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 transition-all ${showCheck ? 'blur-sm pointer-events-none' : ''}`}>
          ...
          <div className="md:col-span-2 space-y-6">
              ...
              <div className="bg-gradient-to-br from-blue-700 to-blue-900 text-white p-6 pb-12">
                ...
                <div className="mt-8 flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <div className="text-6xl font-black tracking-tighter">{t.wr_weather_data}</div>
                      <div className="text-xl font-medium mt-1 text-blue-100">{t.wr_weather_desc}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <button onClick={() => setShowCheck(true)} className="w-full bg-blue-600 text-white p-5 text-xl font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-colors flex justify-center items-center gap-3">
              {t.wr_btn_finish} <span className="text-2xl">→</span>
            </button>
          </div>

          {/* REALISTISCHER 7-TAGE TREND STATS ANZEIGE */}
          <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4 cursor-pointer hover:bg-slate-50" onClick={() => onTrackClick('Result_Radar_Widget')}>
              <span className="text-2xl">🗺️</span>
              <div>
                <div className="text-xs font-bold text-slate-800 uppercase">{t.wr_radar_title}</div>
                <div className="text-xs text-blue-500 underline">{t.wr_radar_text}</div>
              </div>
            </div>
          </div>
        ...
        {showCheck && (
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white border-4 border-charcoal p-6 md:p-10 shadow-diy max-w-xl w-full mx-auto animate-window-pop text-center">
              ...
              <button onClick={() => onComplete({ attentionAnswer: answer })} disabled={!answer} className="w-full bg-jade text-white border-4 border-charcoal p-4 font-black uppercase text-lg shadow-diy disabled:opacity-50">
                {t.wr_check_btn}
              </button>
            </div>
          </div>
